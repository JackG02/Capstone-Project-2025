...
function getCourseraCourses($minor_name, $access_token) {
    ...
    }, array_slice($data['elements'], 0, 2));
}

// Fetch minors with fixed parameter binding
$minors = []; // Default empty minors list

if (!empty($_GET['query']) || !empty($_GET['department'])) {
    ...
    // Execute query only if a valid SQL statement exists
    if (!empty($sql)) {
...
.rvt-flex-md-up.rvt-items-center-md-up {
    margin-bottom: -60px !important; /* Reduce space below profile */
}
...
.minor-name {
        ...
        .search-input {
    width: 100%; /* Make it the same width */
...
<div class="d-flex align-items-center gap-2 mt-3 mt-md-0" style="height: 44px;">
    ...
    <h2>Explore Potential Minors</h2>
    <p>
        Use the search bar to look for minors by name or filter by department.
        Click <strong>"View Details"</strong> to learn more about each minor, and <strong>"Save Minor"</strong> to add it to your profile.
    </p>
</div>
        ...
        <div class="rvt-container-lg rvt-p-tb-xxl">
            <form method="GET" class="rvt-flex rvt-m-bottom-lg" style="width: 100%; display: flex; gap: 15px; flex-wrap: wrap;">
            <input type="text" class="rvt-input search-input" name="query" placeholder="Search minors..." value="<?= htmlspecialchars($_GET['query'] ?? '') ?>">
<select name="department" class="rvt-select search-input">

        <option value="">All Departments</option>
        <?php
        $departments = $pdo->query("SELECT DISTINCT department FROM Minors ORDER BY department ASC")->fetchAll(PDO::FETCH_COLUMN);
    ...
    <button class="rvt-button" type="submit">Search</button>
    <a href="search_minors.php" class="rvt-button rvt-button--secondary">Clear</a>
</form>


            
<?php if (empty($minors)): ?>
    <p style="text-align: center; font-size: 1.2rem; color: #990000; margin-top: 20px;">
        Select filters to see available minors.
    </p>
<?php else: ?>
    <ul class="rvt-list-reset rvt-border-top rvt-m-top-lg">
        <?php foreach ($minors as $minor): ?>
            <li class="rvt-border-bottom rvt-p-tb-xs">
                <span class="minor-name"><?= htmlspecialchars($minor['name']) ?></span>
                ...
                <a href="minor_details.php?minor_id=<?= urlencode($minor['MinorID']) ?>" class="rvt-button rvt-button--primary">
                    View Details
                </a>
                <button class="rvt-button rvt-button--primary save-minor" 
                        data-minor-id="<?= htmlspecialchars($minor['MinorID']) ?>">
                    Save Minor
                </button>
            </li>
        <?php endforeach; ?>
...
<script>
    ...
    document.querySelectorAll(".save-minor").forEach(button => {
        button.addEventListener("click", function () {
            ...
            // Send a POST request to save the minor
            ...
            .then(jsonData => {
                if (jsonData.success) {
                    ...
                    this.innerText = "Saved"; // Change button text
                    this.classList.remove("rvt-button--primary");
                    this.classList.add("rvt-button--success");  
                    this.disabled = true; // Disable button after saving
                } else {
        ...
        function toggleCourses(minorId) {
            document.getElementById(`minor-courses-${minorId}`).classList.toggle("show");
        }

        function toggleRequiredCourses(minorId) {
            document.getElementById(`required-courses-${minorId}`).classList.toggle("show");
        }
    </script>
    <script>
document.addEventListener("DOMContentLoaded", function () {
    function sendPreference(courseName, liked) {
        ...
        .then(data => {
            if (data.success) {
                alert(liked ? "Course Liked!" : "Course Disliked!");
            } else {
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".like-course").forEach(button => {
        button.addEventListener("click", function () {
            ...
            fetch("../epic3/save_course_preference.php", {
                method: "POST",
                body: JSON.stringify({ course_name: courseName, liked: 1 }),
                headers: { "Content-Type": "application/json" }
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    ...
                    button.style.backgroundColor = "#28a745"; // Green color for liked state
