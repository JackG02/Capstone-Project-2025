...
// Fetch pending minor requests (Only Submitted Requests)
$query = "
    SELECT sm.id AS request_id, sm.user_id, sm.minor_id, sm.submitted_at, a.name AS student_name, a.email, m.name AS minor_name, ud.major
    FROM SavedMinors sm
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['request_id'])) {
    ...
    if (!empty($status)) {
        $updateStmt = $pdo->prepare("UPDATE SavedMinors SET status = ? WHERE id = ?");
        if ($updateStmt->execute([$status, $request_id])) {
...
<head>
    ...
    <title>Pending Minor Approvals</title>
...
<body class="rvt-layout">
                ...
                <div class="rvt-header-global__inner">
                    <a class="rvt-lockup" href="adlanding.php">
                        ...
                        <div class="rvt-lockup__body">
                            ...
                            <span class="rvt-lockup__subtitle">Advisor Portal</span>
                        </div>
                    </a>
    ...
    <main id="main-content" class="rvt-layout__wrapper rvt-container-lg">
        <div class="container mt-4">
            <div class="text-center mb-4">
                <h2 class="page-title">Manage Pending Minor Requests</h2>
                <p class="rvt-ts-16" style="max-width: 700px; margin: 0 auto;">
                    This page allows advisors to review student-submitted minor requests. Use the options below to approve or deny each request and help guide students on their academic path.
                </p>
            </div>
            ...
            <!-- Search Bar -->
            <input type="text" id="search" class="form-control search-bar" placeholder="Search students by name...">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <?php if (empty($pendingRequests)): ?>
                        <p class="no-results-message">No pending requests.</p>
                    <?php else: ?>
                        <ul class="list-group" id="student-list">
                            <?php foreach ($pendingRequests as $request): ?>
                                <li class="list-group-item student-item">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <span class="student-name" style="cursor:pointer; color:#0055A5;" onclick="toggleDetails(this)">
                                            <strong><?= htmlspecialchars($request['student_name']) ?></strong> - <?= htmlspecialchars($request['minor_name']) ?>
                                        </span>
                                    ...
                                    <!-- Hidden Details -->
                                    <div class="student-details mt-2" style="display:none; padding-left: 10px;">
                                        <small><strong>Email:</strong> <?= htmlspecialchars($request['email']) ?></small><br>
                                        <small><strong>Major:</strong> <?= htmlspecialchars($request['major'] ?? 'Not provided') ?></small>
                                    </div>
                                </li>
                            <?php endforeach; ?>
                        </ul>
    ...
    <script>
        document.getElementById("search").addEventListener("keyup", function() {
            ...
            let queryParam = searchQuery === "" ? "all" : encodeURIComponent(searchQuery);

            fetch(`search_students.php?query=${queryParam}`)
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        studentList.innerHTML = '<p class="no-results-message">No matching students found.</p>';
                    } else {
                        data.forEach(student => {
                            const listItem = document.createElement("li");
                            listItem.className = "list-group-item d-flex justify-content-between align-items-start flex-column flex-md-row";

                            // Create name + minor span
                            const nameContainer = document.createElement("div");
                            const clickableName = document.createElement("span");
                            clickableName.className = "clickable-name";
                            clickableName.style.cursor = "pointer";
                            clickableName.innerHTML = `<strong>${student.student_name}</strong>`;

                            const minorName = document.createElement("span");
                            minorName.innerHTML = ` - ${student.minor_name}`;

                            nameContainer.appendChild(clickableName);
                            nameContainer.appendChild(minorName);
                    ...
                    `;
                            ...
                            buttonContainer.className = "action-buttons d-flex gap-2 mt-2 mt-md-0";
                    ...
                    `;
                            ...
                            denyForm.className = "d-inline";
                            denyForm.innerHTML = `
                        <input type="hidden" name="request_id" value="${student.request_id}">
                        <button type="submit" name="action" value="deny" class="btn btn-danger btn-sm">Deny</button>
                    `;

                            buttonContainer.appendChild(approveForm);
                            buttonContainer.appendChild(denyForm);
    ...
    <script>
        function toggleDetails(el) {
            const details = el.closest('li').querySelector('.student-details');
            if (details) {
