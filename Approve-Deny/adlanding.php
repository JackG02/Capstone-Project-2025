...
// Fetch logged-in advisor info (for navbar and profile card)
$google_id = $_SESSION['google_id'] ?? null;
$name = 'Advisor';
$email = 'advisor@example.com';

if ($google_id) {
    $stmt = $pdo->prepare("SELECT name, email FROM accounts WHERE id = ?");
...
$initials = strtoupper((isset($name_parts[0][0]) ? $name_parts[0][0] : '') . (isset($name_parts[1][0]) ? $name_parts[1][0] : ''));

// Get count of **Pending** minor requests (not approved or denied)
$pendingQuery = "SELECT COUNT(*) AS pending FROM SavedMinors WHERE status = 'Submitted'";
...
<body class="rvt-layout">
    ...
    <div class="action-section">
        <div class="card-link">
            <h4>Pending Minor Requests</h4>
            <p>View and respond to student-submitted requests for minor approvals.</p>
            <p><strong>Pending Approvals:</strong> <?= $pendingRequests; ?></p>
            <a href="advisor.php" class="card-button">Go to Approvals</a>
        </div>

        <div class="card-link">
            <h4>Dashboard</h4>
            <p>Explore trends and visualizations about student minor selections and demographics.</p>
            <a href="dashboard.php" class="card-button">View Dashboard</a>
        </div>
    </div>
